  ...
  iehfc_server <- function(input, output, session) {
    ...
    source(file.path(system.file("iehfc_app/server_scripts", package = "iehfc"), "duplicates.R"), local = TRUE)
    source(file.path(system.file("iehfc_app/server_scripts", package = "iehfc"), "outliers.R"), local = TRUE)
    source(file.path(system.file("iehfc_app/server_scripts", package = "iehfc"), "enumerator.R"), local = TRUE)
    source(file.path(system.file("iehfc_app/server_scripts", package = "iehfc"), "admin.R"), local = TRUE)
    source(file.path(system.file("iehfc_app/server_scripts", package = "iehfc"), "unit.R"), local = TRUE)
      ...
      output$duplicate_setup <- renderUI({
          card(
              ...
              downloadButton("duplicate_s_exp",
                             label = NULL,
                             shiny::img(src = "www/logo_stata.svg", height = "20px"),
                             class = "btn-light",
                             style = "float: right; margin-left: 10px;"
    ...
    output$outlier_output <- renderUI({
      ...
      output$full_report_dl <- downloadHandler(
          ...
          content = function(file) {
              ...
              rmd_path <- system.file("iehfc_app/server_scripts/template_report.Rmd", package = "iehfc")

              # Render the R Markdown file with parameters
              rmarkdown::render(rmd_path, output_file = file,
                                params = list(
